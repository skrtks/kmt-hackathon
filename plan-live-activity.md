...
Status: Implemented (Kotlin + Swift sources; Xcode widget extension target wiring is a manual step)
...
- Add a `LiveActivityController` interface in `core/PlatformServices.kt`:
  - `start(snapshot: LiveActivitySnapshot)`
  - `update(snapshot: LiveActivitySnapshot)`
  - `end(reason: LiveActivityEndReason)` where reason is one of `SessionEnded`, `Skipped`, `Leaving`, `WatchStopped`, `ScheduleEnded`.
